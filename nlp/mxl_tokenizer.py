...
def MusicXML_to_tokens(mxml_path, bar_major=True, note_name=True, tokenize_chord_symbols=True):
    """
    Converts a MusicXML or MXL file to a list of tokens.
    This updated version supports an arbitrary number of parts.
    
    For bar_major mode, it assumes that all parts are measure-aligned.
    For each measure, a 'bar' token is added, optionally chord tokens,
    and then for each part a part marker (e.g. "P1", "P2", etc.) followed by that measure's tokens.
    
    In non-bar_major mode, tokens for each part are concatenated with a part marker.
    """
    parts, soup = load_MusicXML(mxml_path)
    num_parts = len(parts)

    # Remove the old check; now we support any number of parts.
        ...
        for measure_idx in range(num_measures):
            ...
            if tokenize_chord_symbols and chords is not None:
                ...
                if measure_idx < len(chords):
                    measure_tokens += chords[measure_idx]
            # Process each part in this measure.
            for part_idx, part in enumerate(parts):
                part_marker = f"P{part_idx + 1}"
                measure_tokens.append(part_marker)
                # Pass staff=None so that all parts are handled uniformly.
                measure_tokens += measure_to_tokens(part[measure_idx], soup, staff=None, note_name=note_name)
            tokens += measure_tokens
    else:
        # In non-bar-major mode, process each part separately with a part marker.
        for part_idx, part in enumerate(parts):
            part_marker = f"P{part_idx + 1}"
